 ...
 */
const unsigned char eeprom_header[4] = "DM64";

/**
 * Original name: eeprom_header_bits
 */
const unsigned char eeprom_header_bits[4] = {
    7, // ceil(log2('D'))
    7, // ceil(log2('M'))
    6, // ceil(log2('6'))
    6, // ceil(log2('4'))
 ...
 */
const char _defName[DEFNAME_LEN] = "NEW ";
...
void dm_init_system_mem(void) {
    ...
    for (i = 0; i < 4; i++) {
        ...
        evs_default_name[i] = font2index((const unsigned char*)&_defName[i * 2]);
...
EepRomStatus EepRom_Init(void) {
    ...
    EepRom_InitVars();
    temp = 0;//osEepromProbe(&B_800F3E38); //todo: fix
    if ((temp != 1) && (temp != 2)) {
...
u8* func_80038938(bool arg0) {
    if (!_cached_1332 || arg0) {
        _cached_1332 = 0; //!osEepromLongRead(&B_800F3E38, 0, _cache_1333, sizeof(_cache_1333)); //todo: fix
    }
...
void RecWritingMsg_init(RecordWritingMessage* recMessage, UNK_PTR* arg1) {
    msgWnd_init(&recMessage->messageWnd, arg1, 0x18, 3 /*2 US, 3 JP ?*/, 0, 0);
