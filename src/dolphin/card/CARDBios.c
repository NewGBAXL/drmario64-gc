...
#ifdef DEBUG
const char* __CARDVersion = "<< Dolphin SDK - CARD\tdebug build: Sep  5 2002 ??:??:?? (0x2301) >>";
#else
...
inline static void SetupTimeoutAlarm(CARDControl *card)
{
    OSCancelAlarm(&card->alarm);
    switch (card->cmd[0]) {
        case 0xF2:
...
inline static s32 CARDGetResultCode(s32 chan)
{
    CARDControl *card;
    if (chan < 0 || chan >= 2) {
        return CARD_RESULT_FATAL_ERROR;
...
inline s32 CARDGetSectorSize(s32 chan, u32 *size)
{
