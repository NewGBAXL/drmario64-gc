...
static void stateCheckID()
{
    switch (CurrCommand) {
        case 3:
            ...
            break;
        default:
            if (memcmp(&CurrDiskID, currID, sizeof(DVDDiskID)) != 0) {
                DVDLowStopMotor(cbForStateCheckID1);
            }
            else {
...
static void cbForStateCheckID2(u32 intType)
{
    if (intType == 16) {
        executing->state = -1;
        __DVDStoreErrorCode(0x1234568, executing);
        DVDReset();
        cbForStateError(0);
        return;
    }
    ...
    if (intType & 1) {
        ...
        DVDLowRead(bootInfo->FSTLocation, OSRoundUp32B(BB2.FSTLength), BB2.FSTPosition, &cbForStateReadingFST, bootInfo);
        return;
    }
    DVDLowRequestError(&cbForStateGettingError);
}
